...
torch_device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") # Use "mps" for Mac
...
from Parler_TTS import generate

audio_buffer = BytesIO()

# Move to the start of the BytesIO buffer to read it
audio_buffer.seek(0)

# from streamer import generate
# now you can do
...
#for (sampling_rate, audio_chunk) in generate(text, description, chunk_size_in_s):
  ...
  #sf.write("parler_tts_out.wav", audio_chunk,sampling_rate)


for (sampling_rate, audio_chunk) in generate(text, description, chunk_size_in_s):
    # Stream the audio chunk directly
